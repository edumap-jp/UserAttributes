 ...
 */
	private function __assertMoveSettingTopMenu($result, $userAttrSettingId, $updRow, $updCol, $updWeight, $disabled) {
		...
		if ($disabled) {
			...
			$this->assertNotContains('<a href="" onclick="$(\'form[name=UserAttributeMoveForm' . $userAttrSettingId . 'Top]\')[0].submit()">', $result);
		} else {
			$this->assertNotContains('<li class="disabled">', $result);
			$this->assertContains('<a href="" onclick="$(\'form[name=UserAttributeMoveForm' . $userAttrSettingId . 'Top]\')[0].submit()">', $result);
		}
		$this->assertContains('<span class="glyphicon glyphicon-arrow-up">' . __d('user_attributes', 'Go to Up') . '</span>', $result);
		$this->assertInput('form', 'UserAttributeMoveForm' . $userAttrSettingId . 'Top', '/user_attribute_settings/move/' . $userAttrSettingId, $result);
...
/**
 * moveSettingTopMenu()のテスト
 * [レイアウト＝1列、順序＝2列目の先頭(全て2列)。2列から1列に途中から変更した場合]
 *
 * @return void
 */
...
/**
 * moveSettingTopMenu()のテスト
 * [レイアウト＝1列、順序＝2列目の先頭。2列から1列に途中から変更した場合]
 *
 * @return void
 */
	public function testLayout1AndCol2AndWeight1And() {
	//テストデータ生成
		$viewVars = $this->__viewVars;
		$viewVars['userAttributeLayouts']['1'] = '1';
